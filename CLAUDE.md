...
- `eval/metrics.py` + `eval/benchmark.py` — provider-based benchmark on the gold set (`eval/dataset.jsonl`); rich metrics (per-class F1, macro-F1, confusion matrix, Cohen's kappa, CVE-F1, product recall, latency) → `benchmark_*.csv` + HTML report. Goes through `make_provider` so it measures the SAME models the app runs (local + cloud).
- `enrichment/` — `kev.py` (CISA KEV, cached), `epss.py` (FIRST.org EPSS), `priority.py` (composite 0–100 score from threat_level+CVSS+EPSS+KEV; KEV floors high), `service.py` (`enrich_pending()` — one KEV fetch + one EPSS batch per pass, writes priority back). `pipeline.run_crawl_cycle()` calls it after analysis; failures are swallowed so a crawl never breaks on enrichment.
- `reporting/charts_svg.py` — zero-dependency inline-SVG bar/grouped/heatmap charts. `reporting/briefing.py` — `generate_briefing()` writes a self-contained HTML threat briefing to `OUTPUT_DIR`.
