...
1. **預設模型改為 `qwen2.5:7b`**（已套用到 `config.py`）——分級準確率從 61% → 84%（zero-shot）。
2. **few-shot 已導入 production**（`analyzer/llm.py` 的 `FEWSHOT_EXAMPLES` + `_build_messages`）：每次分析前置 3 筆涵蓋 CRITICAL/LOW/INFO 的範例，與評測 V1 策略一致，預期分級到 ~92%。代價是每次呼叫 token 變多、稍慢；若 D 軌測得 pipeline 吞吐吃緊，可調降範例數或改回 zero-shot。
3. **CVE 以 NVD 為準**：LLM 的 `cve_ids` 僅補充，避免幻覺 CVE 污染 Dashboard 與 Scanner。（production 實測仍會幻覺 CVE-ID，再次佐證此點。）
